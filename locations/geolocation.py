"""
Geolocation resolution logic for converting GPS coordinates to Nepal administrative boundaries.

This module contains the core geolocation logic used by both the legacy views.py endpoint
and the new API endpoint in api_views.py.
"""
...
def resolve_coordinates_to_location(lat, lng):
    """
    ...
    Returns:
        tuple: (result_dict, status_code)
            - result_dict: Dictionary with province, district, municipality data
            - status_code: HTTP status code (200 or 404)
    ...
    if not (26.3 <= lat <= 30.5 and 80 <= lng <= 88.2):
        ...
        return ({'error': _('Location outside Nepal boundaries')}, 404)
    ...
    try:
        ...
        GeolocationAnalytics.track_request(lat, lng, success=False)
        return ({'error': 'Could not determine location'}, 404)
